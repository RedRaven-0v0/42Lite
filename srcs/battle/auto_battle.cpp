...
void auto_battle(Hero &hero, Enemy &enemy)
{
	...
	if (hero.is_alive())
	{
		...
		if (enemy.get_name() == "Anaconda_de_Maxence")
			ExpManager::add_exp(50);
		else if (enemy.get_name() == "Transcende")
			ExpManager::add_exp(50);
		else if (enemy.get_name() == "minishell")
			ExpManager::add_exp(50);
		else
			ExpManager::add_exp(5 * enemy.get_level());
	}
	else
